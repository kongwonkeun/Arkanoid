...
class EventReceiver:
    ...
    is invoked with the event that triggered it. 
    More than one handler can be registered for a given event type.
    """
    def __init__(self):
    ...
    def receive(self):
        """
        Receive the latest list of pygame events (if any) and 
        dispatch them to any registered handlers.
        """
    ...
    def register_handler(self, event_type, *handlers):
        ...
        Args:
            event_type: The pygame event type.
            handlers:
                One or more event handlers. 
                An event handler is a callable that will be called when an event of the given type occurs.
                The callable should accept a single argument, which is the event itself.
        """
    ...
    def unregister_handler(self, *handlers):
        """
        Unregisters one or more event handlers so that they will no longer receive events.
        Args:
            handlers: One or more event handlers to unregister.
        """
        assert len(handlers) > 0
