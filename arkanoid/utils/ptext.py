...
OUTLINE_UNIT = 1 / 24
SHADOW_UNIT = 1 / 18
DEFAULT_ALIGN = "left"  # left, center, right
...
def wrap(text, fontname, fontsize, width = None, widthem = None):
    ...
    if  width is None:
        return texts
    lines = []
    # lines may be split at any space character such that the width of text[:a] is less than width, or
    # at the first space character on the line (after leading spaces), regardless of width.
    for text in texts:
        text = text.rstrip() + " "
...
def _resolveangle(angle):
    ...
    angle %= 360
    return int(
        round(angle / ANGLE_RESOLUTION_DEGREES)) * ANGLE_RESOLUTION_DEGREES
...
def getsurf(
        text, fontname = None, fontsize = None, width = None, widthem = None,
        color = None, background = None, antialias = True, ocolor = None, owidth = None, scolor = None, shadow = None,
    ...
    key = (
        text, fontname, fontsize, width, widthem, color, background, antialias,
        ocolor, opx, scolor, spx, gcolor, alpha, align, lineheight, angle
    )
    ...
    if  angle:
        surf0 = getsurf(
            text, fontname, fontsize, width, widthem, 
            color, background, antialias, ocolor, owidth, scolor, shadow, 
            gcolor, alpha, align, lineheight, cache=cache
        )
        ...
        surf0 = getsurf(
            text, fontname, fontsize, width, widthem, 
            color=color, background=(0, 0, 0, 0), antialias=antialias,
            gcolor=gcolor, align=align, lineheight=lineheight, cache=cache
        )
        ssurf = getsurf(
            text, fontname, fontsize, width, widthem, 
            color=scolor, background=(0, 0, 0, 0), antialias=antialias,
        ...
        osurf = getsurf(
            text, fontname, fontsize, width, widthem, 
            color=ocolor, background=(0, 0, 0, 0), antialias=antialias,
            align=align, lineheight=lineheight, cache=cache
        )
        w0, h0 = surf0.get_size()
        ...
        if  background is None or (len(background) > 3 and background[
            3] == 0) or gcolor is not None:
...
def draw(
        text, pos = None, fontname = None, fontsize = None, antialias = True,
        ...
        center = None, centerx = None, centery = None, width = None, widthem = None, lineheight = None,
        align = None, owidth = None, ocolor = None, shadow = None, scolor = None, gcolor = None, alpha = 1.0,
        anchor = None, angle = 0, surf = _default_surf_sentinel, cache = True
    ):
    ...
    tsurf = getsurf(
        text, fontname, fontsize, width, widthem, color,
        background, antialias,
        ocolor, owidth, scolor, shadow, gcolor, alpha, align,
...
def drawbox(text, rect, fontname = None, lineheight = None, anchor = None, **kwargs):
    ...
    return draw(text, (x, y), fontname=fontname, fontsize=fontsize,
