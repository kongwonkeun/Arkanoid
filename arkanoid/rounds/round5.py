...
from arkanoid.sprites.powerup import (CatchPowerUp, DuplicatePowerUp, ExpandPowerUp, LaserPowerUp, SlowBallPowerUp)
...
class Round5(BaseRound):
    ...
    def _create_bricks(self):
        ...
        """
        bricks = [
            self._blit_brick(Brick(BrickColour.orange, 5, powerup_cls=ExpandPowerUp), 4, 2),
            self._blit_brick(Brick(BrickColour.orange, 5, powerup_cls=LaserPowerUp), 8, 2),
            self._blit_brick(Brick(BrickColour.orange, 5), 5, 3),
            self._blit_brick(Brick(BrickColour.orange, 5, powerup_cls=SlowBallPowerUp), 7, 3),
            self._blit_brick(Brick(BrickColour.orange, 5), 5, 4),
            self._blit_brick(Brick(BrickColour.orange, 5), 7, 4)
        ]
        for y in range(5, 7):
            for x in range(4, 9):
                bricks.append(self._blit_brick(Brick(BrickColour.silver, 5), x, y))
        ...
        for y in range(9, 16):
            for x in range(2, 11):
                if (x, y) not in [
                    ...
                    bricks.append(self._blit_brick(Brick(BrickColour.silver, 5, powerup_cls=powerup_cls), x, y))
