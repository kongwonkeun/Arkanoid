...
class Arkanoid:
    ...
    def main_loop(self):
        """
        Starts the main loop of the program which manages the screen interactions and game play.
    ...
    def _start_game(self, round_no):
        ...
        either by hitting the spacebar, 
        or by entering a specific round number to start at.
        """
...
class StartScreen:
    """
    Used to display the screen shown when the program is first run, and before a game is started.
    Apart from displaying some general information about the game, 
    the start screen is also responsible for capturing user input to decide 
    when to start a game, and which level to start on.
    """
    def __init__(self, on_start):
        ...
        Args:
            on_start:
                Callback invoked when a player starts a new game. 
                The callback should accept a single argument: the round number that the game will start at.
        """
    ...
    def show(self):
        """
        Display the start screen and register event listeners for capturing keyboard input.
...
class Game:
    ...
    def _update_sprites(self):
        """
        Erase the sprites, update their state, and then redraw them on the screen.
        """
        # Erase.
    ...
    def on_brick_collide(self, brick, sprite):
        ...
        In this case a sprite might be the ball, or a laser beam from the laser paddle.
        Args:
            brick: The Brick instance the sprite collided with.
            sprite: The sprite instance that struck the brick.
        """
    ...
    def on_enemy_collide(self, enemy, sprite):
        ...
        In this case a sprite might be the ball, or a laser beam from the laser paddle.
        Args:
            enemy: The Enemy instance the sprite collided with.
            sprite: The sprite instance that struck the enemy.
        """
        enemy.explode()
        self.score += 500
        # Temporarily remove the enemy sprites from the balls to prevent
        # the balls from colliding with the explosion.
        # The enemy sprites are re-added to the balls when they are re-released.
    ...
    def release_enemy(self, enemy):
        ...
        Args:
            enemy: The enemy sprite to release through one of the doors.
        """
    ...
    def _off_screen(self, ball):
        ...
        Args:
            ball: The ball that left the screen.
        """
        if len(self.balls) > 1:
            # There are multiple balls in play, 
            # so just take this ball out of play.
            self.balls.remove(ball)
            self.sprites.remove(ball)
            ball.visible = False
        else:
            # This ball is the last in play, so transition to the BallOffScreenState 
            # which handles end of life.
    ...
    def ball(self):
        ...
        Returns:
            The priamry ball in the game, or None if no balls currently in play.
        """
        try:
            return self.balls[0]
        except IndexError:
...
class BaseState:
    ...
    def update(self):
        """
        Update the state.
        Sub-states must implement this to perform their state specific behaviour. 
...
class GameStartState(BaseState):
    ...
    but before they actually start playing it, e.g. 
    showing an animation sequence.
    """
    def __init__(self, game):
        super().__init__(game)
...
class RoundStartState(BaseState):
    ...
    This state initialises the sprites so they are set up ready for a new round to begin.
    """
    def __init__(self, game):
        super().__init__(game)
    ...
    def update(self):
        """
        Handle the sequence of events that happen at the beginning of 
        a round just before gameplay starts.
        """
...
class RoundRestartState(RoundStartState):
    """
    Specialisation of RoundStartState that handles the behaviour 
    when a round is restarted due to the ball going off screen.
    """
    def __init__(self, game):
        super().__init__(game)
...
class RoundEndState(BaseState):
    """
    This state handles the behaviour when the round ends (is completed successfully).
    """
    def __init__(self, game):
        super().__init__(game)
...
class GameEndState(BaseState):
    """
    This state handles the behaviour when the game ends, either due to all lives being lost, or 
    when the player successfully reaches the very end.
    """
    def __init__(self, game):
        super().__init__(game)
