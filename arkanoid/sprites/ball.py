...
class Ball(pygame.sprite.Sprite):
    ...
    that have been added via add_collidable_sprite().
    Note that the game edges are considered to be collidable sprites.
    The ball will begin its journey using the position and angle specified when the ball is initialised. 
    As the ball collides with sprites, its angle of bounce will be calculated. 
    Furthermore, its speed may increase or decrease but it will never exceed the top_speed.
    When not colliding, the ball will gradually try to settle back to the base_speed.
    Once the ball goes off the screen, the no-args off_screen_callback will be invoked, 
    if one was set when the ball was initialised.
    ...
    def __init__(self, start_pos, start_angle, base_speed, top_speed=15, normalisation_rate=0.02, off_screen_callback=None):
        ...
        If supplied, the off_screen_callback will be invoked whenever the ball leaves the screen. 
        This callable takes a single argument: the ball sprite instance.
        Args:
            start_pos: The starting coordinates of the ball. A 2 element sequence.
            start_angle: The starting angle of the ball in radians.
            base_speed: 
                The baseline speed of the ball. Collisions with objects may
                momentarily increase/decrease the speed of the ball, 
                but the ball will always try to gradually settle back to the base speed.
            top_speed: 
                The maximum permitted speed of the ball. 
                Collisions with objects may increase the speed of the ball,
                but the speed will never go above the top_speed.
            normalisation_rate: 
                The per-frame rate at which the ball is brought back to base speed, 
                should the speed have changed due to collision with an object.
            off_screen_callback: 
                A callable that will be called if the ball goes off the edge of the screen. 
                It takes a single argument: the ball sprite instance.
        """
        super().__init__()
        ...
        # This dictionary is keyed by the collidable sprite. 
        # The value is a 3-element tuple corresponding to the bounce strategy, 
        # speed adjustment and collision callback for that sprite.
    ...
    def add_collidable_sprite(self, sprite, bounce_strategy=None, speed_adjust=0.0, on_collide=None):
        ...
        Args:
            sprite: The collidable sprite.
            bounce_strategy:
                Optional callable that determines how the ball should bounce
                when it collides with the sprite. It takes 2 arguments: 
                the Rect of the object and the Rect of the ball.
            speed_adjust:
                Optional numeric value that will be used to speed up or slow down the the ball. 
                Use a negative value to slow the ball down.
            on_collide:
                Optional callable that will be called when a collision occurs.
                It takes 2 arguments: the sprite the ball struck and the ball that struck it.
        """
    ...
    def remove_collidable_sprite(self, sprite):
        ...
        Args:
            sprite: The collidable sprite to remove.
        """
    ...
    def clone(self, **kwargs):
        """
        Clone the ball creating a new ball with the same collidable sprites as the instance being cloned.
        ...
        This method accepts an optional list of keyword arguments. 
        These, if supplied, will override the values of the ball being cloned.
        Args:
            kwargs:
                Optional keyword arguments that will be passed to the initialiser of the cloned ball 
                overriding the values of the ball being cloned.
        """
    ...
    def update(self):
        ...
        Check whether the ball has collided with anything and 
        if so, update its angle and speed and invoke any associated actions.
        """
    ...
    def _calc_new_angle(self, rects):
        """
        Calculate the default angle of bounce of the ball, 
        given a sequence of rectangles that the ball collided with.
        """
    ...
    def _determine_collide_points(self, rects):
        ...
        Args:
            rects: The sequence of rectagles the ball has collided with.
        Returns:
