...
class Enemy(pygame.sprite.Sprite):
    """
    An enemy is released from the doors in the top edge and slowly travels downwards towards the paddle.
    Its objective is to act as an obstruction and distraction. 
    Enemies will increase the game score when destroyed.
    ...
    def __init__(self, enemy_type, paddle, on_paddle_collide, collidable_sprites, on_destroyed):
        ...
        Args:
            enemy_type: The type of enemy which determines its appearance.
            paddle: The paddle instance.
            on_paddle_collide:
                Callback invoked when the enemy collides with the paddle. 
                It takes 2 arguments: the enemy and the paddle.
            collidable_sprites:
                A list of sprites that can collide with the enemy. 
                These sprites do not destroy the enemy, but cause the enemy to change direction.
            on_destroyed:
                Callback invoked when the enemy is destroyed, either by
                another sprite or when it falls off the bottom of the screen.
                It takes a single argument: the enemy sprite that's been destroyed.
        """
        super().__init__()
    ...
    def _load_animation_sequence(self, filename_prefix):
        ...
        Args:
            filename_prefix: The prefix of the image sequence.
        Returns:
                ...
                if self._area.contains(self.rect):
                    if pygame.sprite.spritecollide(self, [self._paddle], False):
                        ...
                        if sprites_collided:
                            ...
                            # recalculate direction using free movement algorithm.
    ...
    def _calc_direction_collision(self, sprites_collided):
        ...
        Args:
            sprites_collided: A list of sprites that we have collided with.
        Returns:
    ...
    def _calc_direction(self):
        """
        Calculate the direction of travel when the sprite is movin freely (has not collided).
