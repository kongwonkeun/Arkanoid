...
class PowerUp(pygame.sprite.Sprite):
    ...
    def __init__(self, game, brick, png_prefix, speed=DEFAULT_FALL_SPEED):
        ...
        Args:
            ...
            brick: The brick that triggered the powerup to drop.
            png_prefix:
                The png file prefix that will be used to load the image
                sequence for the powerup animation.
            speed: Optional speed at which the powerup drops. Default 3 pixels per frame.
        """
        super().__init__()
        self.game = game
        self._speed = speed
...
class ExtraLifePowerUp(PowerUp):
    """
    This PowerUp applies an extra life to the game when it collides with the Paddle.
    """
...
class SlowBallPowerUp(PowerUp):
    ...
    def deactivate(self):
        """
        Deactivate the SlowBallPowerUp by returning the ball(s) back to their original speed.
        """
        for ball in self.game.balls:
...
class ExpandPowerUp(PowerUp):
    ...
    def deactivate(self):
        """
        Deactivate the ExpandPowerUp by returning the paddle back to its original size.
        """
...
class LaserPowerUp(PowerUp):
    ...
    def deactivate(self):
        """
        Deactivate the LaserPowerUp by turning the paddle back to a normal paddle.
        """
        self.game.paddle.transition(NormalState(self.game.paddle))
        for ball in self.game.balls:
            ball.base_speed -= 1
...
class CatchPowerUp(PowerUp):
    ...
    def _activate(self):
        """
        Add the ability to catch a ball when it collides with the paddle.
        """
    ...
    def deactivate(self):
        """
        Deactivate the CatchPowerUp from preventing the paddle from catching the ball.
        """
