...
class Paddle(pygame.sprite.Sprite):
    ...
    def __init__(self, left_offset=0, right_offset=0, bottom_offset=0, speed=10):
        ...
        Args:
            ...
            right_offset:
                ...
                will restrict the maximum travel of the paddle.
            bottom_offset: The distance the paddle sits above the bottom of the screen.
    ...
    def update(self):
        ...
        if self._move:
            ...
            if self._area_contains(newpos):
                # But only update the position of the paddle if it's within the movable area.
                self.rect = newpos
            else:
                # The new position is not within the screen area based on current speed, 
                # which might leave a small gap. 
                # Adjust the speed until we match the paddle up with the edge of the game area exactly.
                while self._move != 0:
                    if self._move < 0:
                        self._move += 1
                    else:
    ...
    def move_left(self):
        """
        Tell the paddle to move to the left by the speed set when the paddle was initialised.
        """
    ...
    def move_right(self):
        """
        Tell the paddle to move to the right by the speed set when the paddle was initialised.
        """
    ...
    def on_ball_collide(self, paddle, ball):
        ...
        ball_collide_callbacks list. To monitor for ball collisions, add a callback to that list. 
        A callback will be passed the ball instance that collided.
        Args:
            paddle: The paddle that was struck.
            ball: The ball that struck the paddle.
        """
    ...
    def bounce_strategy(paddle_rect, ball_rect):
        ...
        the angle that the ball bounces off the paddle. 
        The angle of bounce is dependent upon where the ball strikes the paddle.
        Note: this function is not tied to the Paddle class but we house it
        here as it seems a reasonable place to keep it.
        Args:
            paddle_rect: The Rect of the paddle.
            ball_rect: The Rect of the ball.
        Returns:
...
class PaddleState:
    """
    A PaddleState represents a particular state of the paddle, in terms of its graphics and behaviour.
    This base class is abstract and concrete sub-states should implement the update() abstract method. 
    The update() method is called repeatedly by the game and is
    where much of the state specific logic should reside, such as animation.
    The enter() and exit() methods are called when the state is entered and exited respectively.
    When the enter() method is called, any previous paddle state is guaranteed to have exited. 
    The enter() method can therefore be used to access any paddle attributes required for sub-state initialisation. 
    Do not use __init__() for this, because a previous paddle state may still
    be in play and you may end up with attribute values you weren't expecting.
    The exit() method is called before a transition to a new state. 
    States should perform any exit behaviour here, such as triggering an animation to return to normal, 
    before calling the no-args on_exit callback passed to the exit() method.
    """
    ...
    def exit(self, on_exit):
        """
        Trigger any state specific exit behaviour before calling the no-args on_exit callable.
        Args:
            on_exit: A no-args callable that will be called when the exit behaviour has completed.
        """
        on_exit()
...
class _PaddlePulsator:
    ...
    def __init__(self, paddle, image_sequence_name):
        """
        Initialise with the name of the image sequence corresponding to each pulsating paddle frame.
        Args:
            paddle: The paddle.
            image_sequence_name: The name of theimage sequence representing each pulsating frame.
        """
        self._paddle = paddle
...
class MaterializeState(PaddleState):
    """
    This special state animates the paddle as it first appears on the  screen.
    After the paddle has materialized, this state automatically transitions to NormalState.
    """
    def __init__(self, paddle):
        super().__init__(paddle)
...
class WideState(PaddleState):
    ...
    def update(self):
        """
        Animate the paddle expanding from normal to wide or shrinking from wide to normal.
        """
...
class LaserState(PaddleState):
    """
    This state represents a laser paddle which is able to fire bullets upwards at the bricks.
    Animation is used to convert from the normal paddle to the laser paddle and vice-versa.
    """
    ...
    def exit(self, on_exit):
        ...
        # Stop monitoring for spacebar presses now that we're leaving the state.
        receiver.unregister_handler(self._fire)

    def _fire(self, event):
        """
        Event handler that fires bullets from the paddle when the spacebar is pressed.
        """
        if event.key == pygame.K_SPACE:
            self._bullets = [bullet for bullet in self._bullets if bullet.visible]
...
class LaserBullet(pygame.sprite.Sprite):
    ...
    def __init__(self, game, position, speed=15):
        ...
        Args:
            ...
            position: The position the bullet starts from.
            speed: Optional speed at which the bullet travels. Default is 15 pixels per frame.
        """
        super().__init__()
    ...
    def update(self):
        """
        Animate the laser bullet moving upwards, and handle any collisions with bricks.
        """
        ...
        if self.visible:
            ...
            if not top_edge_collision:
                ...
                if brick_collide:
                    ...
                    # The game's score is not increased when a laser destroys a brick.
                    brick.value = 0
